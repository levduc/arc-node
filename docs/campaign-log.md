# Campaign log — arc payment lane (2026-06 → 2026-08)

Verbatim history, moved out of CLAUDE.md on 2026-08-25 so the working document
stays small. Nothing here was edited: it is the chronological record of the reth
2.0/2.3 port, the dual-EL payment lane, the builder-separation and deferred-exec
work, and the lean-lane campaign — including the wrong turns and retractions,
which are the most useful part.

The durable conclusions, the run instructions and the landmine list live in
`CLAUDE.md`. Read that first; come here for "why did we conclude X" or "has this
failure been seen before".
...
145ms@1.65MB scales to ~230@2.59). Minus anchor: 925ms -> 2.8 MB/s = 97%% parity. Same
malachite byte-pipe both lanes => parity is the ceiling; lean wins on payments/byte (4.25x):
**3.3x payments/s at 3%% of the admission cost.** NEXT INCREMENT (scoped, proven pattern):
lean deferred anchor — arc_stageBlock at validation (vote-gap exec), anchor = promote.
