#!/usr/bin/env bash
# DEMO-DAY fleet runner (FAST): 10M-preseed fleet with validator3's data on papaduck's fast
# NVMe (/mnt/blockchain.ssd; home disk fsync 30.9ms vs 1.5ms — 20x). Otherwise = demo-fleet.sh.
...
cd "$REPO"
SCEN=soak4; RUN=/tmp/dualel-fleet-fast; mkdir -p "$RUN"
...
verify(){
  ...
  echo "preseed[5M]=${b:-X}"
  [ "$ok" = 1 ] && [ "$b" = "1000000000000000000" ] && echo "✅ FLEET VERIFIED (4 machines, preseed present, val3 on fast NVMe)" || echo "❌ FLEET VERIFY FAILED — fallback: ./experiments/dual-el/demo-empty.sh start"
}
