...
stop(){
  ...
  ids=$(docker ps -aq --filter name=validator); [ -n "$ids" ] && docker rm -f $ids
  for n in 2 3 4; do tss "${RHOST[$n]}" "ids=\$(docker ps -aq --filter name=validator); [ -n \"\$ids\" ] && docker rm -f \$ids; docker run --rm -v /home/papaduck/arc-fleet:/f --user root alpine rm -rf /f/$SCEN 2>/dev/null" || true; done
  docker run --rm -v "$REPO/.quake":/q --user root alpine rm -rf /q/$SCEN
