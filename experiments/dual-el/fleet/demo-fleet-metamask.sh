...
payment_el_cmd(){ # payment_el_cmd <n> <base>
  ...
  --txpool.pending-max-count=200000 --txpool.queued-max-count=200000 --txpool.pending-max-size=512 --txpool.queued-max-size=512 --txpool.basefee-max-count=200000 --txpool.basefee-max-size=512 --txpool.max-account-slots=${PAY_ACCOUNT_SLOTS:-256} --rpc-cache.max-blocks=${PAY_RPC_CACHE_BLOCKS:-200} --rpc-cache.max-receipts=${PAY_RPC_CACHE_RECEIPTS:-200} ${extra_args} && docker network connect arc_testnet_default validator${n}_el_pay"
}
