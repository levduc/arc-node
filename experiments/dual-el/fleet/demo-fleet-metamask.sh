...
payment_el_cmd(){ # payment_el_cmd <n> <base>
  ...
  --txpool.pending-max-count=${PAY_POOL_COUNT:-200000} --txpool.queued-max-count=${PAY_POOL_COUNT:-200000} --txpool.pending-max-size=512 --txpool.queued-max-size=512 --txpool.basefee-max-count=${PAY_POOL_COUNT:-200000} --txpool.basefee-max-size=512 --txpool.max-account-slots=${PAY_ACCOUNT_SLOTS:-256} --rpc-cache.max-blocks=${PAY_RPC_CACHE_BLOCKS:-200} --rpc-cache.max-receipts=${PAY_RPC_CACHE_RECEIPTS:-200} ${extra_args} && docker network connect arc_testnet_default validator${n}_el_pay"
}
...
PYGEN
  ...
  echo "==> [8c/9] lane economics: payment fee FIXED at ${PAY_FIXED_FEE:-20000000000} wei (minBaseFee==maxBaseFee)"
  # retry: on a fresh chain the first controller txs can be dropped in the quorum race
  for i in 1 2 3 4 5; do
    EVM_GAS=30000000 PAY_GAS="$PAY_GAS" PAY_FIXED_FEE="${PAY_FIXED_FEE:-20000000000}" \
