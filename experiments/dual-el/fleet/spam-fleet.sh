...
# Targets default to the 4-machine fleet layout; override EVM_TGTS/PAY_TGTS for other topologies
# (e.g. drop a dead validator's endpoint — the spammer stalls if any target refuses connections).
set -uo pipefail
...
EVM_TGTS=${EVM_TGTS:-"ws://127.0.0.1:8546,ws://100.85.150.119:8646,ws://100.70.62.92:8746,ws://100.86.97.40:8846"}
PAY_TGTS=${PAY_TGTS:-"ws://127.0.0.1:19546,ws://100.85.150.119:19646,ws://100.70.62.92:19746,ws://100.86.97.40:19846"}
...
start(){
  ...
  ( bloatloop(){ while [ ! -f "$STOP" ]; do target/release/spammer ws --targets "$EVM_TGTS" -r "$EVM_RATE" -t 600 -g 2 -a 200 -l --mix guzzler=100 --guzzler-fn-weights "storage-write=100@${SLOTS}" >"$RUN/spam_evm.log" 2>&1; sleep 1; done; }
    payloop(){ while [ ! -f "$STOP" ]; do target/release/spammer ws --targets "$PAY_TGTS" -r "$PAY_RATE" -t 600 -g 8 -a 1000 -l --recipient-pool "$POOL" --mix transfer=100 >"$RUN/spam_pay.log" 2>&1; sleep 1; done; }
