...
g['config']=dict(g['config']); g['config'].pop('osakaTime',None); g['config'].pop('zero7Time',None)
...
PY
  ...
  docker run --rm -v "$PWD/.quake/$SCEN/validator1/reth-pay":/data/reth/execution-data \
    -v "$PWD/.quake/$SCEN/assets":/app/assets gravity_reth:latest \
  ...
  STAGGER=180 PAYMENT_GENESIS=payment-genesis.json TESTNET="$SCEN" bash experiments/dual-el/gravity/launch-payment-els-gravity.sh >"$RUN/paylane.log" 2>&1
  ...
  echo
  echo "  ✅ GRAVITY-RETH payment-lane demo up.  dashboard →  http://localhost:$PORT"
