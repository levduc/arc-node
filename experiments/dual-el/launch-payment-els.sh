...
for i in 1 2 3 4; do sleep ${STAGGER:-0};
  ...
  docker run -d --name "$name" --network "$NET" \
    ...
    node --datadir=/data/reth/execution-data --chain=/app/assets/${PAYMENT_GENESIS:-genesis.json} \
      ...
      --txpool.pending-max-count=${PAY_POOL_COUNT:-200000} --txpool.queued-max-count=${PAY_POOL_COUNT:-200000} --txpool.pending-max-size=512 --txpool.queued-max-size=512 --txpool.basefee-max-count=${PAY_POOL_COUNT:-200000} --txpool.basefee-max-size=512 \
