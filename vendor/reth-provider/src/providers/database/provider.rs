    ...
    for DatabaseProvider<TX, N>
{
    ...
    fn write_hashed_state(&self, hashed_state: &HashedPostStateSorted) -> ProviderResult<()> {
        ...
        if arc_payment_commitment::persistent::enabled() {
            let changes: Vec<(alloy_primitives::B256, Option<jmt::OwnedValue>)> = hashed_state
                .accounts()
                .iter()
                .map(|(k, m)| {
                    (*k, m.map(|a| {
                        arc_payment_commitment::encode_account(
                            a.nonce,
                ...
                .collect();
            arc_payment_commitment::persistent::commit(&changes);
        }
